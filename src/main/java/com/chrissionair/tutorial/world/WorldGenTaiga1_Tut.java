...
public class WorldGenTaiga1_Tut extends WorldGenAbstractTree
{
    private static final String __OBFID = "CL_00000427";

    public WorldGenTaiga1_Tut()
    {
        super(true);// im Gegensatz zu Vanilla, wo Taiga1 NUR in GenLayer vorkommt, aber nicht bei Saplings
    }

    public boolean generate(World par1World, Random par2Random, int par3, int par4, int par5)
    {
        int l = par2Random.nextInt(5) + 7;// Höhe
        int i1 = l - par2Random.nextInt(2) - 3;// Höhe unterer Stamm
        int j1 = l - i1;// Höhe oberer Stamm
        int k1 = 1 + par2Random.nextInt(j1 + 1);// Kronenspanne .. aber wie der Zufall genau aussieht .. testen ..
        boolean flag = true;
        ...
        if (par4 >= 1 && par4 + l + 1 <= 256)
        {
            ...
            else
            {
                ...
                if (isSoil && par4 < 256 - l - 1)
                {
                    ...
                    // Krone
                    for (i2 = par4 + l; i2 >= par4 + i1; --i2)// von top bis Beginn unterer Stamm
                    {
                        for (j2 = par3 - i3; j2 <= par3 + i3; ++j2)// x
                        {
                            int j3 = j2 - par3;// aktuelle Spanne

                            for (int k2 = par5 - i3; k2 <= par5 + i3; ++k2)// y
                            {
                                int l2 = k2 - par5;// aktuelle Spanne
                        ...
                        if (i3 >= 1 && i2 == par4 + i1 + 1)// wenn unterer Stamm erreicht ist, dann 1 kleiner
                        {
                            --i3;
                        }
                        else if (i3 < k1)
