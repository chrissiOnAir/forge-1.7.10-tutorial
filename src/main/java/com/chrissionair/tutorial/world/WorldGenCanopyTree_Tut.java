...
public class WorldGenCanopyTree_Tut extends WorldGenAbstractTree
{
    private static final String __OBFID = "CL_00000430";

    public WorldGenCanopyTree_Tut(boolean p_i45461_1_)
    {
        super(p_i45461_1_);
    }
    ...
    {
        int l = par2Random.nextInt(3) + par2Random.nextInt(2) + 6;// Stamm
        boolean flag = true;
        ...
        if (par4 >= 1 && par4 + l + 1 <= 256)
        {
            ...
            else
            {
                ...
                if (isSoil && par4 < 256 - l - 1)
                {
                    ...
                    int j3 = par2Random.nextInt(4);// Richtung
                    j1 = l - par2Random.nextInt(4);// unterer Stamm
                    k1 = 2 - par2Random.nextInt(3);// Biegung
                    int k3 = par3;
                    int l1 = par5;
                    int i2 = 0;
                    int j2;
                    ...
                    for (j2 = 0; j2 < l; ++j2)
                    {
                        ...
                        if (j2 >= j1 && k1 > 0)// sachte Biegung in der Höhe
                        {
                    ...
                    // Top Krone, inkl. letztes x und z: mit y Lücke .. 
                    for (j2 = -2; j2 <= 0; ++j2)
                    {
                        for (k2 = -2; k2 <= 0; ++k2)
                        {
                    ...
                    for (j2 = -1; j2 <= 2; ++j2)
                    {
                        for (k2 = -1; k2 <= 2; ++k2)
                        {
                        	// um den unteren Stamm
                            if ((j2 < 0 || j2 > 1 || k2 < 0 || k2 > 1) && par2Random.nextInt(3) <= 0)
                            {
                                int l3 = par2Random.nextInt(3) + 2;
                                int l2;

                                // vom top hinunter ein paar logs, orientiert am unteren Stamm (par3 und 5)
                                for (l2 = 0; l2 < l3; ++l2)
                                {
                                ...
                                // ok, folgende Kronen sind irgendwie unlogisch, denn sie orientieren sich am obersten x und z
                                // k3, l1 sind höchstes x und z
