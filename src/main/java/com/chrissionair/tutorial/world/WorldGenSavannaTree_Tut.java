...
public class WorldGenSavannaTree_Tut extends WorldGenAbstractTree
{
    ...
    {
    	// l ist Höhe des Stamms
        int l = par2Random.nextInt(3) + par2Random.nextInt(3) + 5;
        boolean flag = true;

        // check ob überhaupt Platz ist, wobei b0 ganz oben einen Umfang von 2 hat
        if (par4 >= 1 && par4 + l + 1 <= 256)
        {
            ...
            else
            {
                ...
                if (isSoil && par4 < 256 - l - 1)
                {
                    ...
                    int k3 = par3;
                    int l1 = par5;
                    int i2 = 0; // wird letzte Höhe sein
                    int j2; // von 0 bis zur Höhe kleiner des Stamms; neues relatives y
                    int k2; // das neue absolute y, mit Höhe des Stamms wachsend
                    ...
                    // macht eine erste Krone über dem letzten Stammblock, im Quadrat
                    for (j2 = -1; j2 <= 1; ++j2)
                    {
                        for (k2 = -1; k2 <= 1; ++k2)
                        {
                    ...
                    // eine Ebene darunter, also höchster Stamm, eine Quadrat ohne Ecken
                    for (j2 = -3; j2 <= 3; ++j2)
                    {
                        for (k2 = -3; k2 <= 3; ++k2)
                        {
                    ...
                    k3 = par3; // Zurücksetzen x und z
                    l1 = par5;
                    j2 = par2Random.nextInt(4); // neue Richtung

                    if (j2 != j3)
                    {
                        k2 = j1 - par2Random.nextInt(2) - 1; // neuer Beginn Verzweigung; achtung: letztes k2 (z) überschrieben
                        int l3 = 1 + par2Random.nextInt(3); // wie weit Verzweigung reicht
                        i2 = 0; // die letzte Höhe
                        int i3; // neues absolutes y
                        int l2; // neues relatives y

                        // dadurch dass l2 mit l3 verknüpft ist, kann der Stamm nur soweit wie die weiteste Verzweigung wachsen
                        ...
                        if (i2 > 0)
                        {
                        	// neue Krone on top; achtung wieder l2 und i3 überschrieben
                        	for (l2 = -1; l2 <= 1; ++l2)
                            {
                                for (i3 = -1; i3 <= 1; ++i3)
                                {
    ...
    // platziert den Leavesblock, aber nicht über Stamm
