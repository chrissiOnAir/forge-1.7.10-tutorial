...
public class WorldGenMegaPineTree_Tut extends WorldGenHugeTrees_Tut
{
    ...
    {
        int l = this.func_150533_a(par2Random);// neue rel maxHöhe
    ...
    // y-Wert und Spanne für Top Krone; achtung Wert_4 ist abs Höhe, Wert_5 Spanne
    private void func_150541_c(World p_150541_1_, int p_150541_2_, int p_150541_3_, int p_150541_4_, int p_150541_5_, Random p_150541_6_)
    {
        ...
        {
            int l1 = p_150541_4_ - k1;// rel restHöhe bewachsener Bereich
            // der Mechanismus für die mit der Höhe veränderten Spanne ist relativ kompliziert .. weil so dicht programmiert
            int i2 = p_150541_5_ + MathHelper.floor_float((float)l1 / (float)i1 * 3.5F);// wird kleiner
            // i2 == j1 && (k1 & 1) == 0: die beiden wesentlichen Kriterien für die Stufen
            this.func_150535_a(p_150541_1_, p_150541_2_, k1, p_150541_3_, i2 + (l1 > 0 && i2 == j1 && (k1 & 1) == 0 ? 1 : 0), p_150541_6_);
            j1 = i2;
        }
    }

    // folgende 3 Funktionen sind scheinbar nur dafür da, Dirt im Bodenbereich zu setzen (weil unter Baum? bleibt Dirt oder wird Podzol?) 
